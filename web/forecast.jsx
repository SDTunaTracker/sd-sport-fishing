...
const SD_LAT = 32.72, SD_LNG = -117.23;

function _fmtPacific(date) {
...
function _findMoonTransitMs(dateStr) {
  const dayStart = new Date(dateStr + 'T00:00:00').getTime();
  let maxAlt = -Infinity, bestMs = null;
  for (let i = 0; i < 144; i++) {
    const ms = dayStart + i * 10 * 60 * 1000;
    const alt = SunCalc.getMoonPosition(new Date(ms), SD_LAT, SD_LNG).altitude;
    if (alt > maxAlt) { maxAlt = alt; bestMs = ms; }
  }
  return bestMs;
}

function _mkWin(centerMs, halfMin, type, strength, label, icon) {
  const hw = halfMin * 60 * 1000;
  return { type, strength, label, icon, start: new Date(centerMs - hw), end: new Date(centerMs + hw) };
}

function computeSolunar(dateStr) {
  if (!dateStr || typeof SunCalc === 'undefined') return null;
  const noon      = new Date(dateStr + 'T12:00:00');
  const sunTimes  = SunCalc.getTimes(noon, SD_LAT, SD_LNG);
  const moonTimes = SunCalc.getMoonTimes(noon, SD_LAT, SD_LNG);
  const moonIllum = SunCalc.getMoonIllumination(noon);
  const transitMs = _findMoonTransitMs(dateStr);

  const wins = [];

  if (sunTimes.dawn && sunTimes.goldenHourEnd)
    wins.push({ type: 'sun', strength: 'minor', label: 'Dawn', icon: '🌅',
      start: sunTimes.dawn, end: sunTimes.goldenHourEnd });

  if (moonTimes.rise)
    wins.push(_mkWin(moonTimes.rise.getTime(), 45, 'moon', 'minor', 'Moonrise', '🌙'));

  if (transitMs) {
    wins.push(_mkWin(transitMs, 60, 'moon', 'major', 'Moon Overhead', '🌕'));
    wins.push(_mkWin(transitMs + 12.37 * 3600000, 60, 'moon', 'major', 'Moon Underfoot', '🌑'));
  }

  if (moonTimes.set)
    wins.push(_mkWin(moonTimes.set.getTime(), 45, 'moon', 'minor', 'Moonset', '🌙'));

  if (sunTimes.goldenHour && sunTimes.dusk)
    wins.push({ type: 'sun', strength: 'minor', label: 'Dusk', icon: '🌆',
      start: sunTimes.goldenHour, end: sunTimes.dusk });

  const dayStart = new Date(dateStr + 'T00:00:00').getTime();
  const dayEnd   = dayStart + 86400000;
  const mid = w => (w.start.getTime() + w.end.getTime()) / 2;

  return {
    windows: wins.filter(w => w.start && w.end && mid(w) >= dayStart && mid(w) < dayEnd)
                 .sort((a, b) => a.start - b.start),
    moonIllumPct: Math.round(moonIllum.fraction * 100),
...
function SolunarWidget({ dateStr }) {
  const sol = useMemo(() => {
    try { return computeSolunar(dateStr); } catch(e) { return null; }
  }, [dateStr]);
  ...
  return (
    <div className="fc-section-card">
      <div className="fc-section-hd" style={{marginBottom: 12}}>
        <h2 className="fc-section-title" style={{marginBottom: 0}}>Best Fishing Windows</h2>
        <span className="fc-sol-moon-badge">{moonEmoji(sol.moonIllumPct)} {sol.moonIllumPct}% lit</span>
      </div>

      <div className="fc-sol-ref-row">
        <span>☀️ Sunrise {_fmtPacific(sol.sunrise) || '—'}</span>
        <span>🌅 Sunset {_fmtPacific(sol.sunset) || '—'}</span>
        <span>🌙 Moonrise {_fmtPacific(sol.moonrise) || 'none'}</span>
        <span>🌙 Moonset {_fmtPacific(sol.moonset) || 'none'}</span>
      </div>

      <div className="fc-sol-windows">
        {sol.windows.map((w, i) => {
          const isMajor  = w.strength === 'major';
          const isPassed = w.end < now;
          const isActive = w.start <= now && w.end >= now;
          return (
            <div key={i} className={[
              'fc-sol-row',
              isMajor  ? 'fc-sol-major'  : '',
              isActive ? 'fc-sol-active' : '',
              isPassed ? 'fc-sol-passed' : '',
            ].filter(Boolean).join(' ')}>
              <span className="fc-sol-icon">{w.icon}</span>
              <div className="fc-sol-info">
                <span className="fc-sol-label">{w.label}</span>
                {isMajor && <span className="fc-sol-badge">MAJOR</span>}
              </div>
              <span className="fc-sol-time">
                {_fmtPacific(w.start)}–{_fmtPacific(w.end)}
              </span>
            </div>
          );
        })}
      ...
      <div className="fc-sol-note">
        Major windows occur when the moon is directly overhead or underfoot — peak feeding activity per solunar theory. Minor windows align with moonrise/moonset and dawn/dusk.
      </div>
    </div>
  );
}
...
function ForecastView({ navigate }) {
  ...
  return (
    <React.Fragment>
      ...
      {/* Solunar windows */}
      <SolunarWidget dateStr={today.date}/>
